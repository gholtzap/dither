...
The goal is to continuously turn repeated patterns and implicit concepts into simple, reliable, well-tested building blocks.
 ...
 ## Image Processing Invariants
  ...
  - This is a desktop-first, non-destructive image editor.
  - Never modify or overwrite the source image.
  - Browser/WebView canvas may render previews but must never produce final exports.
       - Final exports must render from the original full-resolution image.
  - Preserve source bit depth, color profile, alpha, and metadata whenever the output
  format supports them.
  - Never silently perform lossy conversion, reduce bit depth, or discard metadata;
  require explicit user confirmation.
                                                      - Use a high-bit-depth, color-managed native processing pipeline.
                        - Default to lossless output formats and compression.
  - Do not claim universal format support; detect capabilities and report unsupported
  files clearly.